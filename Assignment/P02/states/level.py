...
class Level(tools.State):
    ...
    def load_map(self):
        ###Load Map from Json file (ex: level1 -> level1.json)
        map_file = 'level_' + str(self.game_info[c.LEVEL_NUM]) + '.json'
        file_path = os.path.join('data', 'maps', map_file)
        f = open(file_path)
        self.map_data = json.load(f)
        f.close()
        
    def setup_background(self):
        ##set Background Image on level screen
    ...
    def change_map(self, index, type):
        ###change map display when the event 
    ...
    ### Check enemy state(when they appear)
    ...
    def update_all_sprites(self, keys):
        if self.player.dead:
            ...
            if self.current_time - self.death_timer > 3000:
                ...
                self.done = True
        elif self.game_info[c.SCORE] >= 1000 * self.game_info[c.LEVEL_NUM]:
            levelup = pg.mixer.Sound('resources/sound/level.wav')
            levelup.play()
    ...
    def check_player_x_collisions(self):
        ...
        if box:
            ...
            if self.player.invincible:
                ...
                levelup = pg.mixer.Sound('resources/sound/coin.wav')
                levelup.play()
    ...
    def update_game_info(self):
        if self.player.dead:
            ### Play dead sound
            deadSound = pg.mixer.Sound('resources/sound/death.wav')
            deadSound.play()
        ...
        if self.overhead_info.time == 0:
            ...
            if self.game_info[c.LEVEL_NUM] > 3:
                self.game_info[c.LEVEL_NUM] = 1
